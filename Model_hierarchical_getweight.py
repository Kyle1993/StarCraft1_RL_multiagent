...
class Commander_Actor(nn.Module):
    def __init__(self, state_size, command_size, rnn_insize=400, rnn_outsize=100):
        ...
        self.birnn = nn.RNN(input_size=rnn_insize, hidden_size=rnn_outsize, num_layers=1, batch_first=True,
                            bidirectional=True)
        self.init_weights()
    ...
    def get_weight(self):
        previous_weight1,previous_weight2 = self.birnn.all_weights[0][1],self.birnn.all_weights[1][1]
        current_weight1,current_weight2 = self.birnn.all_weights[0][0],self.birnn.all_weights[1][0]
        previous_weight = torch.mean(torch.abs(previous_weight1))+torch.mean(torch.abs(previous_weight2))
        current_weight = torch.mean(torch.abs(current_weight1))+torch.mean(torch.abs(current_weight2))
        return previous_weight.data[0],current_weight.data[0]
...
class Commander_Critic(nn.Module):
    def __init__(self, state_size, command_size, batch_size, rnn_insize=200, rnn_outsize=50):
        ...
        self.h0 = Variable(torch.zeros(2, batch_size, rnn_outsize),
                           requires_grad=False)  # (num_layer*num_driection)*bs*rnn_ooutsize
...
class Unit_Actor(nn.Module):
    def __init__(self, state_size, command_size, action_size, hidden_size=200):
        ...
        self.fc1 = nn.Linear(state_size + command_size, action_size)
        self.init_weights()
    ...
    def get_weight(self):
        command_weight = torch.mean(torch.abs(self.fc1.weight[:, -self.command_size:]))
        state_weight = torch.mean(torch.abs(self.fc1.weight[:,:-self.command_size]))
        return command_weight.data[0],state_weight.data[0]
...
class DDPG(object):
    ...
    def get_command(self, states):
        ...
        command = self.commander_actor_target(group_states,
                                              self.commander_actor_h0.repeat(1, int(self.config.UNIT_BATCH_SIZE / 8),
                                                                             1)).contiguous()
