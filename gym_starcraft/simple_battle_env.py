...
import gym_starcraft.starcraft_env as sc
import random
...
class SimpleBattleEnv(sc.StarCraftEnv):
    def __init__(self, server_ip, server_port, MYSELF_NUM, ENEMY_NUM, ACTION_NUM=3, DISTANCE_FACTOR=10,
                 ...
                 DIE_REWARD=2e-3, HEALTH_REWARD_WEIGHT=4, DONE_REWARD_WEIGHT=1, MY_HEALTH_WEIGHT=1,
                 ENEMY_HEALTH_WEIGHT=4, FOCUS_WEIGHT=5, FRAME_SKIP=2, MAX_STEP=700, speed=0, self_play=False):
        ...
        self.DONE_REWARD_WEIGHT = DONE_REWARD_WEIGHT
    ...
    def compute_reward_separately(self):
        ...
        for uid,unit in self.current_state[0].items():
            done_reward = self.DONE_REWARD_WEIGHT * self._check_done() * (self.ENEMY_NUM-sum(self.enemy_alive.values()))
            # done_reward = self.DONE_REWARD_WEIGHT * self._check_done() * 1000
            if unit.die:
                rewards[uid] = self.DIE_REWARD + done_reward
            else:
                enemy_hp_reward,enemy_shield_reward = 0,0

                # # for mariens to calcuate reward
                # target_hp_reward,target_shield_reward = 0,0
                # if unit.targetUnitId != -1 and unit.attacking:
                #     enemy = self.current_state[1][unit.targetUnitId]
                #     enemy_hp_reward = enemy.delta_health
                #     enemy_shield_reward = enemy.delta_shield
                ...
                # focus_reward = self.FOCUS_WEIGHT*(target_hp_reward+target_shield_reward)

                # for dragoons to calcuate reward
                for enemyid,enemyagent in self.current_state[1].items():
                    enemy_hp_reward += enemyagent.delta_health
                    enemy_shield_reward += enemyagent.delta_shield
                health_reward = self.ENEMY_HEALTH_WEIGHT*(enemy_hp_reward+enemy_shield_reward) - self.MY_HEALTH_WEIGHT*(max(0,unit.delta_health)+max(0,unit.delta_shield))
                focus_reward = 0

                # win_reward = self.win * float(alive)
                range_reward = -0.5*self.DIE_REWARD*self.range_reward(unit,low=15,high=int(unit.groundRange*2))

                rewards[uid] = self.HEALTH_REWARD_WEIGHT * health_reward + done_reward + range_reward + focus_reward
    ...
    def _check_done(self):
        ...
        if len(self.state.units[0]) == 0 or len(self.state.units[1]) == 0:
            if len(self.state.units[0]) > 0 and len(self.state.units[1]) == 0:
                ...
                self.advanced_termination = True
            return True
        if self.episode_steps >= self.step_limit:
